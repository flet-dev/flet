...
class QueryString(UrlComponents):
    ...
    Constructor:
            `page` takes `Page` class as an argument and extracts URL automatically\n

    Methods:
            Public:
                `get()` method takes `key` as an argument and returns value according to key. (Ex: .../?name=Joe -> `get('name')` -> `Joe`)\n
