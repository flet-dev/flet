...
Defaults to `"upload"` when not specified.
...
3. `FLET_ANDROID_SIGNING_KEY_ALIAS`
4. `"upload"`
...
`[tool.flet.android.manifest_application]`. Keys are written as
`android:<key>="<value>"` in the `AndroidManifest.xml`.
...
In the [template](index.md#template-source)'s `AndroidManifest.xml`, it will be translated accordingly into this:
...
```bash
flet build apk --android-permissions permission=True|False ... --android-features feature_name=True|False
```
...
```bash
flet build apk --android-permissions android.permission.INTERNET=False
```
