...
class Badge(BaseControl):
    ...
    If the label is not provided, the badge is shown as a filled circle of [`small_size`](#small_size) diameter. 

    If `label` is provided, the label is a StadiumBorder shaped badge with height equal to [`large_size`](#large_size).
