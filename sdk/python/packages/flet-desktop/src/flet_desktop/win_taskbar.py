...
def _propvariant_from_string(value):
    # InitPropVariantFromString is a header-inline helper (not exported from
    # propsys.dll), so replicate it: a VT_LPWSTR PROPVARIANT owning a
    # CoTaskMem-allocated string copy (PropVariantClear frees it).
    buf = ctypes.c_void_p()
...
def apply_relaunch_props_async(pid):
    """
    Stamp AppUserModel window properties on the Flet client window.

    Waits (on a daemon thread) for the client process to show its top-level
    ...
    and it is an existing executable path, the rest is derived from it.
