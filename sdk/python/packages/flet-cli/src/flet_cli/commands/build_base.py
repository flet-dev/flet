...
class BaseBuildCommand(BaseFlutterCommand):
    ...
    def customize_icons(self):
        ...
        if self.target_platform == "linux":
            ...
            if linux_icon_src.is_file():
                ...
                # The icon-theme directory is named after the icon's real
                # size, so CMake (which cannot measure the image) reads it
                # from here.
    ...
    def resolve_icon_theme_size(png_size: Optional[tuple[int, int]]) -> str:
        ...
        # Square sizes listed in hicolor-icon-theme's index.theme.
        hicolor_sizes = {
            16, 22, 24, 28, 32, 36, 42, 48, 64, 72, 96,
