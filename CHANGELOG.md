...
* **The HTML loading animation is gone from the Flet web client, and `icons/loading-animation.png` no longer overrides anything.** It ran before Flutter's first frame, which made web startup a three-stage sequence — HTML animation, then Flutter's boot screen, then the app — and only the boot screen is configurable. Dropping your own `icons/loading-animation.png` into the assets directory, which the docs previously described, now has no effect; use [`[tool.flet.boot_screen]`](https://flet.dev/docs/publish#boot_screen) instead by @FeodorFitsner.
