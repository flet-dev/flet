...
* Fix `flet build windows` failing on non-UTF-8 system locales (e.g. Simplified-Chinese Windows, code page 936/GBK) with `warning C4819` escalated to `error C2220` while compiling `serious_python_windows_plugin.cpp`. A non-ASCII character in a plugin source comment couldn't be decoded under the system code page, and the Flutter template's `/WX` (warnings-as-errors) turned it fatal. Fixed upstream in `serious_python` 4.3.3 (the character is removed and the Windows plugin now compiles with `/utf-8`), now pinned in the `flet build` template ([#6686](https://github.com/flet-dev/flet/issues/6686)) by @FeodorFitsner.
