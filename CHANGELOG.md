...
* An app embedded with [`FletApp`][flet.FletApp] can now be driven by, and report back to, the app hosting it — enough for a host to simulate a window around a guest rather than merely display it. `FletApp` gains `route`, `title`, `window_state` and `media_padding`, each two-way where that makes sense: setting `route` navigates the guest and `on_route_change` fires when the guest navigates itself; `page.title` in the guest raises `on_title_change` instead of renaming the real OS window around it; `window_state` pushes simulated window geometry and flags down, and `on_window_event` reports what the guest asked its own window to do. `Page.embedded` is `True` inside a guest, so an app can skip anything that assumes it owns its window. The guest's `page.width`/`page.height` and `page.media.padding` now describe the area `FletApp` gives it rather than the host's real window, and `page.window` is wired to the host: `page.window.maximized = True`, `page.window.close()` and the rest become requests the host may honour or ignore, and the host's answer arrives back as the usual `WindowEvent`s (`resize`, `maximize`, `move`, ...), so an app needs no embedded-specific branch. A root app is unaffected — the real window service runs only when the page owns a window, which also fixes a guest having the host's geometry stamped over its window state on every resize by @FeodorFitsner.
