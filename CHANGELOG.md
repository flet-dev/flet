...
* Fix a `flet build web` app showing a blank page forever when its Python program fails to start. The Pyodide worker reports a startup failure — a bad import, a missing dependency, an app archive that will not download — by rejecting the connection, and `FletBackend.connect()` answered that the way it answers any connect failure: by reconnecting. That is right when a server is still coming up and useless here, because the same program fails identically every time, so the app sat on an empty boot screen while a fresh Python worker re-downloaded the runtime every 10 seconds, forever. Worse, the reconnect overwrote the captured error with the string `"Loading..."`, so the real cause was destroyed before anything could show it — and if the retries ever did run out, `"Loading..."` was reported as the error. The transport now raises `FletAppStartupException` for a failure of the app itself, which settles on the error and renders it on the boot screen instead of retrying; a plain unreachable-peer failure still reconnects exactly as before, and now keeps the real error for the give-up path. Two things that hid the cause are fixed alongside it: the worker's fatal init error was logged with `console.log` rather than `console.error`, so a dev tools console filtered to Errors — the obvious way to look for a cause — showed nothing at all; and the worker called `flet_js.send()` unguarded after a failed startup, so the only visible error was a misleading `TypeError: flet_js.send is not a function` pointing at `python-worker.js` instead of at the Python traceback ([#5876](https://github.com/flet-dev/flet/issues/5876)) by @ndonkoHenri.
