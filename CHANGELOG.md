...
* Add configurable built-in and custom controls to `flet-video`, plus `Video.take_screenshot()` for capturing video frames by @ndonkoHenri.
...
* Fix control diffing for controls nested inside `@value` dataclass objects so they keep the nearest control parent/page context, and restore optional structured properties that are cleared to `None` and later set again by @ndonkoHenri.
