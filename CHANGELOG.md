...
* **The [Icons](https://flet.dev/docs/types/icons) and [CupertinoIcons](https://flet.dev/docs/types/cupertinoicons) reference pages now show the icons.** Both pages listed every member of an 8,825- and a 1,322-member class through the generic API renderer, so each icon got a heading, a wrench symbol and a `NAME: IconData` signature box — 9.2 MB of HTML that never showed a single glyph, which made the one question the pages exist to answer ("what does `DELETE_SHARP` look like?") unanswerable without running an app. They are now searchable galleries that draw each icon with the same font the Flet client renders with: Flutter's `MaterialIcons` from the SDK pinned in `.fvmrc`, and `CupertinoIcons` from the version pinned in `client/pubspec.lock`, so the docs cannot show a glyph the app does not draw. Filter Material icons by style (Filled, Outlined, Rounded, Sharp), and click any icon to copy its `ft.Icons.NAME`. Existing deep links such as `#flet.Icons.ARROW_UPWARD` still resolve, and now filter the grid to exactly that icon. The page renders a few screens of icons up front and extends as you scroll, so it stays usable on a phone rather than making the browser parse all 8,825 tiles before it can draw anything; searching always covers the whole set regardless of how much is on screen. The icon names, the packed values the protocol sends, and the codepoints the docs draw with are all emitted from one parse of Flutter's `icons.dart` by `.github/scripts/generate_icons.py`, with a new `--verify` in CI so the three cannot drift apart across Flutter bumps ([#6838](https://github.com/flet-dev/flet/issues/6838), [#6843](https://github.com/flet-dev/flet/pull/6843)) by @ndonkoHenri.
