...
* **`WebView` now works on Windows and Linux.** These were the two platforms `flet-webview` shipped without — the extension rendered an "not yet supported on this platform" placeholder there, because `webview_flutter` has no Windows or Linux implementation and no plans for one. Windows is now backed by [`webview_all_windows`](https://pub.dev/packages/webview_all_windows) (Edge WebView2) and Linux by [`webview_all_linux`](https://pub.dev/packages/webview_all_linux) (WebKitGTK), while Android, iOS, macOS and web stay on `webview_flutter` exactly as before — these two packages each declare a single platform, so nothing of theirs is registered anywhere else. Every `WebView` property, method and event is supported on the new platforms, `on_scroll` included, which is still unavailable on macOS. Two caveats are worth knowing before you ship: on Linux the WebView is a native GTK widget rather than a Flutter texture, so other controls cannot be drawn over it, non-rectangular clips do not apply, and a scale, rotation or skew transform hides it entirely; and Linux needs `libwebkit2gtk-4.1-0` at runtime. Because that library is linked into the whole binary rather than loaded on demand, the Linux clients Flet publishes include the WebView only in the `full` flavor, and only for Debian 12 / Ubuntu 22.04 and newer — the `light` flavor and the older-distro builds omit it, so they keep running on machines without WebKitGTK. Windows needs the Edge WebView2 runtime, which ships with Windows 11 ([#6530](https://github.com/flet-dev/flet/issues/6530)) by @ndonkoHenri.
