...
* **App icon and desktop integration for `flet build linux`.** Linux was the one platform `flet build` shipped without an icon — `flutter_launcher_icons` has no Linux generator, so `assets/icon.png` was silently ignored and built apps ran with a generic icon. The resolved icon (`icon_linux.png`, falling back to `icon.png` or the default Flet icon) is now bundled as `data/app_icon.png` and set as the window icon on startup, which taskbars and window switchers pick up on X11 and XWayland. Because Wayland has no window-icon protocol — desktops resolve icons from an installed desktop entry matching the app id — the bundle also ships a ready-to-install freedesktop tree: `share/applications/<bundle_id>.desktop` (name from `--product`, comment from `--description`, `StartupWMClass` set) plus the icon at `share/icons/hicolor/256x256/apps/<bundle_id>.png`, and the runner now sets its program name to the bundle ID (matching upstream Flutter's runner template) so the running app maps to that entry on both Wayland and X11. See the new [App icon](https://flet.dev/docs/publish/linux#app-icon) docs ([#2269](https://github.com/flet-dev/flet/issues/2269)) by @ndonkoHenri.
...
* Fix `flet build --description` (and `project.description` from `pyproject.toml`) never reaching the generated project: the build passed the value under a template key no file consumed (`description`), while the templates read `project_description` — which was always its empty default. The description now lands in the Flutter project's `pubspec.yaml`, the web app's `index.html` meta description and PWA `manifest.json`, and the new Linux desktop entry's `Comment=` — properly escaped in each format, so a description containing quotes or newlines can no longer produce an unparsable `pubspec.yaml`/`manifest.json` or an invalid desktop entry ([#2269](https://github.com/flet-dev/flet/issues/2269)) by @ndonkoHenri.
