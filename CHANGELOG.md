...
* Fix Windows apps packaged with `flet pack` showing a second taskbar identity named "Flet description", whose pin launches a blank client window instead of the app ([#6767](https://github.com/flet-dev/flet/discussions/6767)). Two independent defects stacked up. First, the AppUserModelID fix from [#6403](https://github.com/flet-dev/flet/pull/6403) never actually ran in released builds: it lives in a PyInstaller runtime hook that PyInstaller discovers through `rthooks.dat`, and that file — plain data, not a `.py` module — was silently missing from the `flet-cli` wheel because no `package-data` declaration covered it (PyInstaller ignores a missing `rthooks.dat` without a warning; the fix only ever worked from editable installs, which is how it passed verification). The wheel now ships it. Second, even with the hook running, `SetCurrentProcessExplicitAppUserModelID` only fixes taskbar *grouping* — the taskbar name, icon and pin target are resolved through the shell's relaunch properties, and with none set they fall back to the window's process image: the cached `flet.exe`. What looked like the raw-path AUMID working in earlier testing was residue in the shell's app-resolver cache from prior experiments, which also explains why [#6421](https://github.com/flet-dev/flet/pull/6421) appeared verified. `flet_desktop` now stamps `System.AppUserModel.ID`/`RelaunchCommand`/`RelaunchDisplayNameResource`/`RelaunchIconResource` on the client window right after launch (new `flet_desktop.win_taskbar` module, pure ctypes), driven by env vars the runtime hook sets. The client window is matched by its window class rather than visibility, so apps started hidden (`AppView.FLET_APP_HIDDEN`) are stamped immediately instead of missing a visibility deadline, and the stamping worker holds a process handle so it stops when the client exits and a recycled PID can never be stamped. Verified end-to-end on Windows 11 — single taskbar entry, correct right-click name, and pins that relaunch the packaged app — including from paths longer than 128 characters and containing spaces, which the window property store accepts even though the process-level `SetCurrentProcessExplicitAppUserModelID` documents a 128-character/no-spaces limit by @ndonkoHenri.

* Fix `flet pack`-patched desktop clients and the vanilla client shadowing each other through the shared `~/.flet/client/` cache. The cache directory was keyed by flavor and version only, and `ensure_client_cached()` returns any existing directory untouched — so a packaged app whose bundled client was patched with a custom icon and metadata would silently run whatever same-version client happened to be cached first (and vice versa: a dev run could end up branded as a previously-run packaged app). Bundled client archives are now content-fingerprinted — `flet pack` writes a `<archive>.sha256` sidecar (hash plus size, so a stale sidecar next to a replaced archive is detected and re-hashed) and `ensure_client_cached()` keys the cache directory by that fingerprint — so each distinct client gets its own cache entry. To keep `~/.flet/client` bounded, client archives are built deterministically (fixed timestamps, sorted entries), so rebuilds with unchanged content reuse the same fingerprint, and superseded fingerprint directories unused for 30 days are garbage-collected with a rename-before-delete guard that leaves any running app's client untouched; concurrent first-run extractions of the same archive no longer crash the losing process by @ndonkoHenri.
