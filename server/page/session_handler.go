...
func newSession(page *model.Page, id string, clientIP string, clientUserAgent string,
	...
	windowTop string, windowLeft string, isPwa string, isWeb string, isDebug string, platform string, platformBrightness string, media string) *model.Session {
	s := &model.Session{}
	s.Page = page
	s.ID = id
	s.ClientIP = clientIP
